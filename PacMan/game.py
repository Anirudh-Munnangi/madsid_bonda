...
class Game(object):
    def __init__(self):
        #self.BONDA_SPRITE = pygame.image.load('bonda.png').convert_alpha()
        ...
        for i, row in enumerate(enviroment()):
            for j, item in enumerate(row):
                if item != 0:
                    self.dots_group.add(Ellipse(j*32+12,i*32+12,ORANGE,15,15))
                    #self.dots_group.add(self.BONDA_SPRITE)
    ...
    def display_frame(self,screen):
        ...
        if self.game_over:
            if self.about:
                self.display_message(screen,"It is an arcade Game of Madhav Sahu & the bondas !!")
            ...
            text = self.font.render("Bondas: " + str(self.score),True,GREEN)
